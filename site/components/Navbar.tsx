...
export function Navbar({ whatsappHref }: NavbarProps) {
  return (
    <header className="sticky top-0 z-50 border-b border-white/8 bg-[#090D16]/80 backdrop-blur-xl">
      <div className="mx-auto max-w-7xl px-6 py-4">
        <div className="flex items-center justify-between gap-4">
          ...
          <nav className="hidden items-center gap-8 md:flex">
            {navLinks.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className={`text-sm font-semibold transition-all duration-300 hover:text-white ${
          ...
          <Link
            href={whatsappHref}
            className="inline-flex items-center justify-center gap-2 rounded-full bg-[#FACC15] px-5 py-3 text-sm font-bold text-[#0B0F1A] shadow-[0_20px_50px_-20px_rgba(250,204,21,0.75)] transition-all duration-300 hover:scale-105 hover:bg-[#fde047]"
          >
            Solicitar demo
