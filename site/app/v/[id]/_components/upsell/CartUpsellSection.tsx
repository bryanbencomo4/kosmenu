...
export function CartUpsellSection({ suggestions, formatPrice, onAdd, onDismiss }: CartUpsellSectionProps) {
  ...
  return (
    <div className="rounded-[24px] border border-[var(--menu-border)] bg-[var(--menu-surface)] p-4">
      <h5 className="text-sm font-black text-[var(--menu-text)]">Completa tu pedido</h5>
        ...
        {suggestions.map((suggestion) => (
          <div key={suggestion.productId} className="flex items-center gap-3 rounded-[18px] border border-[var(--menu-border)] p-2.5">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-[13px] font-bold text-[var(--menu-text)]">{suggestion.name}</p>
              <p className="text-[12px] font-semibold text-[var(--menu-text-muted)]">{formatPrice(suggestion.price)}</p>
            </div>
            <button
              type="button"
            ...
            <button
              ...
              className="grid h-7 w-7 shrink-0 place-items-center rounded-full text-[var(--menu-text-muted)]"
            >
