...
type BundleRailProps = {
  items: BundleRailItem[];
  formatPrice: (amount: number) => string;
  onAdd: (bundleId: string) => void;
  titleStyle?: CSSProperties;
};

const CARD_STEP_PX = 222;

/**
 * Real commercial offers — fixed items at a fixed price, never a relabeled
 * carousel of arbitrary products.
 */
export function BundleRail({ items, formatPrice, onAdd, titleStyle }: BundleRailProps) {
  ...
  const onPointerDown = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    dragRef.current = { pointerId: event.pointerId, startX: event.clientX, startScrollLeft: el.scrollLeft, moved: false };
    setIsDragging(true);
      ...
      <div className="flex items-end justify-between gap-3 px-4 sm:px-6">
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            style={{ backgroundColor: 'var(--menu-surface)', borderColor: 'var(--menu-border)', color: 'var(--menu-text)' }}
          >
            ...
            >
              ...
              <div className="p-3">
                ...
                <p className="mt-1 line-clamp-2 text-[11px] leading-4" style={{ color: 'var(--menu-text-muted)' }}>
                  {item.description?.trim() || item.itemsLabel}
                </p>
