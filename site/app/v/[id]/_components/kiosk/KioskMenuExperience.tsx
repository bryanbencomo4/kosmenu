...
export function KioskMenuExperience({
  ...
  return (
    <section className="relative flex min-h-[100dvh] flex-col overflow-x-hidden bg-[var(--menu-background)] text-[var(--menu-text)]">
      ...
      <header className="sticky top-0 z-30 bg-[color-mix(in_srgb,var(--menu-background)_92%,white)] px-5 py-3 backdrop-blur-[6px]">
        <div className="mx-auto flex w-full max-w-5xl items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h2 className={`${titleFont.className} truncate text-lg font-extrabold tracking-[-0.03em] text-[#111827]`}>
          ...
          {logoUrl ? (
            <KioskImage
              src={logoUrl}
              alt=""
              className="h-11 w-11 rounded-[14px] bg-white shadow-[0_8px_24px_rgba(15,23,42,0.06)]"
            />
          ) : (
            <div
              className="grid h-11 w-11 place-items-center rounded-[14px] text-sm font-extrabold text-white"
        ...
        <label className="mx-auto mt-3 flex h-12 w-full max-w-5xl items-center gap-2 rounded-[18px] bg-white px-3.5 shadow-[0_8px_30px_rgba(15,23,42,0.06)]">
      ...
      <div className="relative z-10 mx-auto w-full max-w-5xl flex-1 px-5 py-4 pb-36">
        ...
        >
          {screen === 'products' ? 'Arma tu pedido a tu ritmo.' : '¿Qué se te antoja hoy?'}
        </p>
        ...
        {screen === 'categories' ? (
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              {visibleCategories.map((category, index) => (
                <button
                  ...
                  className="kiosk-card overflow-hidden rounded-[22px] bg-white text-left shadow-[0_8px_30px_rgba(15,23,42,0.06)] transition duration-200 ease-out hover:-translate-y-0.5 hover:shadow-[0_12px_34px_rgba(15,23,42,0.09)]"
                ...
                >
                  ...
                  <div className="px-3 py-3">
                    <p className={`${titleFont.className} text-sm font-extrabold leading-5 text-[#111827]`}>
                      {category.name}
                    </p>
                  </div>
                </button>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {visibleProducts.map((product, index) => (
              <article
                key={product.id}
                className="kiosk-card flex overflow-hidden rounded-[22px] bg-white shadow-[0_8px_30px_rgba(15,23,42,0.06)]"
                style={{ animationDelay: `${Math.min(index, 8) * 40}ms` }}
              >
                <div className="h-32 w-32 shrink-0 bg-slate-50 sm:h-36 sm:w-36">
                  {product.imageUrl ? (
                ...
                <div className="flex min-w-0 flex-1 flex-col justify-between p-3.5">
                  <div>
                    <h3 className={`${titleFont.className} text-base font-extrabold leading-5 text-[#111827]`}>
                      {product.name}
                    </h3>
                    {product.description ? (
                      <p className="mt-1 line-clamp-2 text-xs leading-4 text-slate-500">{product.description}</p>
                    ) : null}
                  </div>
                  <div className="mt-3 flex items-center justify-between gap-2">
                    <p className="text-sm font-bold text-[#111827]">{product.priceLabel}</p>
                    <button
                      type="button"
                      disabled={!product.available}
                      onClick={() => onAddProduct(product.id)}
                      className="rounded-[14px] px-3.5 py-2 text-xs font-bold text-white transition duration-200 hover:-translate-y-0.5 disabled:opacity-40 disabled:hover:translate-y-0"
                      style={{ backgroundColor: 'var(--menu-primary)' }}
                    >
                      Agregar
                    </button>
      ...
      {cartCount > 0 ? (
        <div className="fixed inset-x-0 bottom-0 z-40 px-4 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
          <div className="mx-auto flex w-full max-w-5xl items-center gap-3 rounded-[22px] bg-white px-4 py-3 shadow-[0_8px_30px_rgba(15,23,42,0.08)]">
            <div className="min-w-0 flex-1">
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">Tu pedido</p>
              <p className={`${titleFont.className} truncate text-lg font-extrabold text-[#111827]`}>
            ...
            <button
              type="button"
              onClick={onPay}
              className="inline-flex min-h-12 items-center gap-2 rounded-[16px] px-5 text-sm font-bold text-white"
              style={{ backgroundColor: 'var(--menu-primary)' }}
            >
      ...
      {addedPrompt ? (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/35 p-4 sm:items-center">
          <div className="w-full max-w-md rounded-[22px] bg-white p-5 text-[#111827] shadow-[0_16px_40px_rgba(15,23,42,0.12)]">
            <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500">Agregado</p>
            ...
            <div className="mt-5 grid gap-2">
              ...
              <button
                ...
                className="min-h-12 rounded-[16px] bg-slate-50 text-sm font-bold text-slate-600"
              >
                Seguir agregando
              </button>
            </div>
...
function EmptyState({ message }: { message: string }) {
  return (
    <div className="grid min-h-[36vh] place-items-center rounded-[22px] bg-white px-6 text-center text-sm font-medium text-slate-500 shadow-[0_8px_30px_rgba(15,23,42,0.06)]">
      {message}
    </div>
  );
}
