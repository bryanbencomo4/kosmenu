...
begin
  ...
  -- Avoid dependency on gen_random_bytes() availability in search_path.
  v_token := lower(
    md5(random()::text || clock_timestamp()::text || v_order_id || coalesce(v_actor::text, '')) ||
    md5(clock_timestamp()::text || random()::text || coalesce(p_phone, '') || coalesce(p_note, ''))
  );

  -- Keep SHA-256 hash format expected by web API lookup.
